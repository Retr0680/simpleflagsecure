.class public Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;
.super Ljava/lang/Object;
.source "BiometricFrameworkStatsLogger.java"


# static fields
.field private static final TAG:Ljava/lang/String; = "BiometricFrameworkStatsLogger"

.field private static final sInstance:Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;


# direct methods
.method public static synthetic $r8$lambda$66kbtPMDMvRPXfV6O_t300MEgkw(Ljava/lang/Integer;)I
    .locals 0

    .line 0
    invoke-static {p0}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->lambda$toProtoWakeReasonDetails$1(Ljava/lang/Integer;)I

    move-result p0

    return p0
.end method

.method public static synthetic $r8$lambda$abVN1EGVqWySyl1Pu5PYaB3KO0A(I)Z
    .locals 0

    .line 0
    invoke-static {p0}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->lambda$toProtoWakeReasonDetails$2(I)Z

    move-result p0

    return p0
.end method

.method public static synthetic $r8$lambda$bkHYB9CplNafjbDRFHoMLLhJOck(Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;Lcom/android/server/biometrics/log/OperationContextExt;IIIZJIZILjava/lang/Float;)V
    .locals 0

    .line 0
    invoke-direct/range {p0 .. p11}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->lambda$authenticate$0(Lcom/android/server/biometrics/log/OperationContextExt;IIIZJIZILjava/lang/Float;)V

    return-void
.end method

.method static constructor <clinit>()V
    .locals 1

    .line 42
    new-instance v0, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;

    invoke-direct {v0}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;-><init>()V

    sput-object v0, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->sInstance:Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;

    return-void
.end method

.method private constructor <init>()V
    .locals 0

    .line 45
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method private static foldType(I)I
    .locals 1
    .param p0, "foldType"    # I

    .line 314
    packed-switch p0, :pswitch_data_0

    .line 322
    const/4 v0, 0x0

    return v0

    .line 316
    :pswitch_0
    const/4 v0, 0x2

    return v0

    .line 318
    :pswitch_1
    const/4 v0, 0x1

    return v0

    .line 320
    :pswitch_2
    const/4 v0, 0x3

    return v0

    nop

    :pswitch_data_0
    .packed-switch 0x1
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method

.method public static getInstance()Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;
    .locals 1

    .line 49
    sget-object v0, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->sInstance:Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;

    return-object v0
.end method

.method private synthetic lambda$authenticate$0(Lcom/android/server/biometrics/log/OperationContextExt;IIIZJIZILjava/lang/Float;)V
    .locals 12
    .param p1, "operationContext"    # Lcom/android/server/biometrics/log/OperationContextExt;
    .param p2, "statsModality"    # I
    .param p3, "statsAction"    # I
    .param p4, "statsClient"    # I
    .param p5, "isDebug"    # Z
    .param p6, "latency"    # J
    .param p8, "authState"    # I
    .param p9, "requireConfirmation"    # Z
    .param p10, "targetUserId"    # I
    .param p11, "ambientLightLux"    # Ljava/lang/Float;

    .line 110
    nop

    .line 111
    invoke-virtual/range {p11 .. p11}, Ljava/lang/Float;->floatValue()F

    move-result v11

    .line 110
    move-object v0, p0

    move-object v1, p1

    move v2, p2

    move v3, p3

    move/from16 v4, p4

    move/from16 v5, p5

    move-wide/from16 v6, p6

    move/from16 v8, p8

    move/from16 v9, p9

    move/from16 v10, p10

    invoke-virtual/range {v0 .. v11}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->authenticate(Lcom/android/server/biometrics/log/OperationContextExt;IIIZJIZIF)V

    .line 112
    return-void
.end method

.method private static synthetic lambda$toProtoWakeReasonDetails$1(Ljava/lang/Integer;)I
    .locals 1
    .param p0, "i"    # Ljava/lang/Integer;

    .line 182
    invoke-virtual {p0}, Ljava/lang/Integer;->intValue()I

    move-result v0

    return v0
.end method

.method private static synthetic lambda$toProtoWakeReasonDetails$2(I)Z
    .locals 1
    .param p0, "i"    # I

    .line 183
    if-eqz p0, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    return v0
.end method

.method private static orientationType(I)I
    .locals 1
    .param p0, "rotation"    # I

    .line 300
    packed-switch p0, :pswitch_data_0

    .line 310
    const/4 v0, 0x0

    return v0

    .line 308
    :pswitch_0
    const/4 v0, 0x4

    return v0

    .line 306
    :pswitch_1
    const/4 v0, 0x3

    return v0

    .line 304
    :pswitch_2
    const/4 v0, 0x2

    return v0

    .line 302
    :pswitch_3
    const/4 v0, 0x1

    return v0

    nop

    :pswitch_data_0
    .packed-switch 0x0
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method

.method private sanitizeLatency(J)J
    .locals 2
    .param p1, "latency"    # J

    .line 282
    const-wide/16 v0, 0x0

    cmp-long v0, p1, v0

    if-gez v0, :cond_0

    .line 283
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    const-string/jumbo v1, "found a negative latency : "

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v0, p1, p2}, Ljava/lang/StringBuilder;->append(J)Ljava/lang/StringBuilder;

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    const-string v1, "BiometricFrameworkStatsLogger"

    invoke-static {v1, v0}, Landroid/util/Slog;->w(Ljava/lang/String;Ljava/lang/String;)I

    .line 284
    const-wide/16 v0, -0x1

    return-wide v0

    .line 286
    :cond_0
    return-wide p1
.end method

.method private static sessionType(B)I
    .locals 2
    .param p0, "reason"    # B

    .line 290
    const/4 v0, 0x2

    const/4 v1, 0x1

    if-ne p0, v1, :cond_0

    .line 291
    return v0

    .line 293
    :cond_0
    if-ne p0, v0, :cond_1

    .line 294
    return v1

    .line 296
    :cond_1
    const/4 v0, 0x0

    return v0
.end method

.method static toProtoWakeReason(Lcom/android/server/biometrics/log/OperationContextExt;)I
    .locals 2
    .param p0, "operationContext"    # Lcom/android/server/biometrics/log/OperationContextExt;

    .line 189
    invoke-virtual {p0}, Lcom/android/server/biometrics/log/OperationContextExt;->getWakeReason()I

    move-result v0

    .line 190
    .local v0, "reason":I
    packed-switch v0, :pswitch_data_0

    .line 210
    const/4 v1, 0x0

    return v1

    .line 208
    :pswitch_0
    const/16 v1, 0x9

    return v1

    .line 206
    :pswitch_1
    const/16 v1, 0x8

    return v1

    .line 204
    :pswitch_2
    const/4 v1, 0x7

    return v1

    .line 202
    :pswitch_3
    const/4 v1, 0x6

    return v1

    .line 200
    :pswitch_4
    const/4 v1, 0x5

    return v1

    .line 198
    :pswitch_5
    const/4 v1, 0x4

    return v1

    .line 196
    :pswitch_6
    const/4 v1, 0x3

    return v1

    .line 194
    :pswitch_7
    const/4 v1, 0x2

    return v1

    .line 192
    :pswitch_8
    const/4 v1, 0x1

    return v1

    nop

    :pswitch_data_0
    .packed-switch 0x1
        :pswitch_8
        :pswitch_7
        :pswitch_6
        :pswitch_5
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method

.method private static toProtoWakeReasonDetails(Landroid/hardware/biometrics/common/AuthenticateReason;)I
    .locals 1
    .param p0, "reason"    # Landroid/hardware/biometrics/common/AuthenticateReason;

    .line 215
    if-eqz p0, :cond_0

    .line 216
    invoke-virtual {p0}, Landroid/hardware/biometrics/common/AuthenticateReason;->getTag()I

    move-result v0

    packed-switch v0, :pswitch_data_0

    goto :goto_0

    .line 218
    :pswitch_0
    invoke-virtual {p0}, Landroid/hardware/biometrics/common/AuthenticateReason;->getFaceAuthenticateReason()I

    move-result v0

    invoke-static {v0}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->toProtoWakeReasonDetailsFromFace(I)I

    move-result v0

    return v0

    .line 221
    :cond_0
    :goto_0
    const/4 v0, 0x0

    return v0

    nop

    :pswitch_data_0
    .packed-switch 0x1
        :pswitch_0
    .end packed-switch
.end method

.method static toProtoWakeReasonDetails(Lcom/android/server/biometrics/log/OperationContextExt;)[I
    .locals 3
    .param p0, "operationContext"    # Lcom/android/server/biometrics/log/OperationContextExt;

    .line 180
    invoke-virtual {p0}, Lcom/android/server/biometrics/log/OperationContextExt;->toAidlContext()Landroid/hardware/biometrics/common/OperationContext;

    move-result-object v0

    .line 181
    .local v0, "ctx":Landroid/hardware/biometrics/common/OperationContext;
    iget-object v1, v0, Landroid/hardware/biometrics/common/OperationContext;->authenticateReason:Landroid/hardware/biometrics/common/AuthenticateReason;

    invoke-static {v1}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->toProtoWakeReasonDetails(Landroid/hardware/biometrics/common/AuthenticateReason;)I

    move-result v1

    invoke-static {v1}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v1

    invoke-static {v1}, Ljava/util/stream/Stream;->of(Ljava/lang/Object;)Ljava/util/stream/Stream;

    move-result-object v1

    new-instance v2, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger$$ExternalSyntheticLambda1;

    invoke-direct {v2}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger$$ExternalSyntheticLambda1;-><init>()V

    .line 182
    invoke-interface {v1, v2}, Ljava/util/stream/Stream;->mapToInt(Ljava/util/function/ToIntFunction;)Ljava/util/stream/IntStream;

    move-result-object v1

    new-instance v2, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger$$ExternalSyntheticLambda2;

    invoke-direct {v2}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger$$ExternalSyntheticLambda2;-><init>()V

    .line 183
    invoke-interface {v1, v2}, Ljava/util/stream/IntStream;->filter(Ljava/util/function/IntPredicate;)Ljava/util/stream/IntStream;

    move-result-object v1

    .line 184
    invoke-interface {v1}, Ljava/util/stream/IntStream;->toArray()[I

    move-result-object v1

    .line 181
    return-object v1
.end method

.method private static toProtoWakeReasonDetailsFromFace(I)I
    .locals 1
    .param p0, "reason"    # I

    .line 225
    packed-switch p0, :pswitch_data_0

    .line 247
    const/4 v0, 0x0

    return v0

    .line 245
    :pswitch_0
    const/16 v0, 0xa

    return v0

    .line 243
    :pswitch_1
    const/16 v0, 0x9

    return v0

    .line 241
    :pswitch_2
    const/16 v0, 0x8

    return v0

    .line 239
    :pswitch_3
    const/4 v0, 0x7

    return v0

    .line 237
    :pswitch_4
    const/4 v0, 0x6

    return v0

    .line 235
    :pswitch_5
    const/4 v0, 0x5

    return v0

    .line 233
    :pswitch_6
    const/4 v0, 0x4

    return v0

    .line 231
    :pswitch_7
    const/4 v0, 0x3

    return v0

    .line 229
    :pswitch_8
    const/4 v0, 0x2

    return v0

    .line 227
    :pswitch_9
    const/4 v0, 0x1

    return v0

    :pswitch_data_0
    .packed-switch 0x1
        :pswitch_9
        :pswitch_8
        :pswitch_7
        :pswitch_6
        :pswitch_5
        :pswitch_4
        :pswitch_3
        :pswitch_2
        :pswitch_1
        :pswitch_0
    .end packed-switch
.end method


# virtual methods
.method public acquired(Lcom/android/server/biometrics/log/OperationContextExt;IIIZIII)V
    .locals 19
    .param p1, "operationContext"    # Lcom/android/server/biometrics/log/OperationContextExt;
    .param p2, "statsModality"    # I
    .param p3, "statsAction"    # I
    .param p4, "statsClient"    # I
    .param p5, "isDebug"    # Z
    .param p6, "acquiredInfo"    # I
    .param p7, "vendorCode"    # I
    .param p8, "targetUserId"    # I

    .line 56
    nop

    .line 59
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isCrypto()Z

    move-result v3

    .line 66
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getId()I

    move-result v10

    .line 67
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getReason()B

    move-result v0

    invoke-static {v0}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->sessionType(B)I

    move-result v11

    .line 68
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isAod()Z

    move-result v12

    .line 69
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isDisplayOn()Z

    move-result v13

    .line 70
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getDockState()I

    move-result v14

    .line 71
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getOrientation()I

    move-result v0

    invoke-static {v0}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->orientationType(I)I

    move-result v15

    .line 72
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getFoldState()I

    move-result v0

    invoke-static {v0}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->foldType(I)I

    move-result v16

    .line 73
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getOrderAndIncrement()I

    move-result v17

    .line 74
    invoke-static/range {p1 .. p1}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->toProtoWakeReason(Lcom/android/server/biometrics/log/OperationContextExt;)I

    move-result v18

    .line 56
    const/16 v0, 0x57

    const/4 v9, -0x1

    move/from16 v1, p2

    move/from16 v4, p3

    move/from16 v5, p4

    move/from16 v8, p5

    move/from16 v6, p6

    move/from16 v7, p7

    move/from16 v2, p8

    invoke-static/range {v0 .. v18}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIIZIIIIZIIIZZIIIII)V

    .line 75
    return-void
.end method

.method public authenticate(Lcom/android/server/biometrics/log/OperationContextExt;IIIZJIZIF)V
    .locals 23
    .param p1, "operationContext"    # Lcom/android/server/biometrics/log/OperationContextExt;
    .param p2, "statsModality"    # I
    .param p3, "statsAction"    # I
    .param p4, "statsClient"    # I
    .param p5, "isDebug"    # Z
    .param p6, "latency"    # J
    .param p8, "authState"    # I
    .param p9, "requireConfirmation"    # Z
    .param p10, "targetUserId"    # I
    .param p11, "ambientLightLux"    # F

    .line 81
    nop

    .line 84
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isCrypto()Z

    move-result v3

    .line 88
    move-object/from16 v0, p0

    move-wide/from16 v1, p6

    invoke-direct {v0, v1, v2}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->sanitizeLatency(J)J

    move-result-wide v7

    .line 92
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getId()I

    move-result v12

    .line 93
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getReason()B

    move-result v4

    invoke-static {v4}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->sessionType(B)I

    move-result v13

    .line 94
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isAod()Z

    move-result v14

    .line 95
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isDisplayOn()Z

    move-result v15

    .line 96
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getDockState()I

    move-result v16

    .line 97
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getOrientation()I

    move-result v4

    invoke-static {v4}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->orientationType(I)I

    move-result v17

    .line 98
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getFoldState()I

    move-result v4

    invoke-static {v4}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->foldType(I)I

    move-result v18

    .line 99
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getOrderAndIncrement()I

    move-result v19

    .line 100
    invoke-static/range {p1 .. p1}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->toProtoWakeReason(Lcom/android/server/biometrics/log/OperationContextExt;)I

    move-result v20

    .line 101
    invoke-static/range {p1 .. p1}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->toProtoWakeReasonDetails(Lcom/android/server/biometrics/log/OperationContextExt;)[I

    move-result-object v21

    .line 102
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getIsMandatoryBiometrics()Z

    move-result v22

    .line 81
    const/16 v0, 0x58

    const/4 v10, -0x1

    move/from16 v1, p2

    move/from16 v4, p4

    move/from16 v9, p5

    move/from16 v6, p8

    move/from16 v5, p9

    move/from16 v2, p10

    move/from16 v11, p11

    invoke-static/range {v0 .. v22}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIIZIZIJZIFIIZZIIIII[IZ)V

    .line 103
    return-void
.end method

.method public authenticate(Lcom/android/server/biometrics/log/OperationContextExt;IIIZJIZILcom/android/server/biometrics/log/ALSProbe;)V
    .locals 12
    .param p1, "operationContext"    # Lcom/android/server/biometrics/log/OperationContextExt;
    .param p2, "statsModality"    # I
    .param p3, "statsAction"    # I
    .param p4, "statsClient"    # I
    .param p5, "isDebug"    # Z
    .param p6, "latency"    # J
    .param p8, "authState"    # I
    .param p9, "requireConfirmation"    # Z
    .param p10, "targetUserId"    # I
    .param p11, "alsProbe"    # Lcom/android/server/biometrics/log/ALSProbe;

    .line 109
    new-instance v0, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger$$ExternalSyntheticLambda0;

    move-object v1, p0

    move-object v2, p1

    move v3, p2

    move v4, p3

    move/from16 v5, p4

    move/from16 v6, p5

    move-wide/from16 v7, p6

    move/from16 v9, p8

    move/from16 v10, p9

    move/from16 v11, p10

    invoke-direct/range {v0 .. v11}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger$$ExternalSyntheticLambda0;-><init>(Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;Lcom/android/server/biometrics/log/OperationContextExt;IIIZJIZI)V

    const/4 v1, 0x0

    move-object/from16 v2, p11

    invoke-virtual {v2, v0, v1}, Lcom/android/server/biometrics/log/ALSProbe;->awaitNextLux(Ljava/util/function/Consumer;Landroid/os/Handler;)V

    .line 113
    return-void
.end method

.method public enroll(IIIIJZFII)V
    .locals 12
    .param p1, "statsModality"    # I
    .param p2, "statsAction"    # I
    .param p3, "statsClient"    # I
    .param p4, "targetUserId"    # I
    .param p5, "latency"    # J
    .param p7, "enrollSuccessful"    # Z
    .param p8, "ambientLightLux"    # F
    .param p9, "source"    # I
    .param p10, "templateId"    # I

    .line 119
    nop

    .line 122
    move-wide/from16 v0, p5

    invoke-direct {p0, v0, v1}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->sanitizeLatency(J)J

    move-result-wide v5

    .line 119
    const/16 v2, 0xb8

    const/4 v8, -0x1

    move v3, p1

    move/from16 v4, p4

    move/from16 v7, p7

    move/from16 v9, p8

    move/from16 v10, p9

    move/from16 v11, p10

    invoke-static/range {v2 .. v11}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIIJZIFII)V

    .line 128
    return-void
.end method

.method public enumerated(III[I[I)V
    .locals 6
    .param p1, "statsModality"    # I
    .param p2, "targetUserId"    # I
    .param p3, "result"    # I
    .param p4, "templateIdsHal"    # [I
    .param p5, "templateIdsFramework"    # [I

    .line 142
    const/16 v0, 0x3b1

    move v1, p1

    move v2, p2

    move v3, p3

    move-object v4, p4

    move-object v5, p5

    .end local p1    # "statsModality":I
    .end local p2    # "targetUserId":I
    .end local p3    # "result":I
    .end local p4    # "templateIdsHal":[I
    .end local p5    # "templateIdsFramework":[I
    .local v1, "statsModality":I
    .local v2, "targetUserId":I
    .local v3, "result":I
    .local v4, "templateIdsHal":[I
    .local v5, "templateIdsFramework":[I
    invoke-static/range {v0 .. v5}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIII[I[I)V

    .line 148
    return-void
.end method

.method public error(Lcom/android/server/biometrics/log/OperationContextExt;IIIZJIII)V
    .locals 23
    .param p1, "operationContext"    # Lcom/android/server/biometrics/log/OperationContextExt;
    .param p2, "statsModality"    # I
    .param p3, "statsAction"    # I
    .param p4, "statsClient"    # I
    .param p5, "isDebug"    # Z
    .param p6, "latency"    # J
    .param p8, "error"    # I
    .param p9, "vendorCode"    # I
    .param p10, "targetUserId"    # I

    .line 154
    nop

    .line 157
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isCrypto()Z

    move-result v3

    .line 163
    move-object/from16 v0, p0

    move-wide/from16 v1, p6

    invoke-direct {v0, v1, v2}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->sanitizeLatency(J)J

    move-result-wide v9

    .line 165
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getId()I

    move-result v12

    .line 166
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getReason()B

    move-result v4

    invoke-static {v4}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->sessionType(B)I

    move-result v13

    .line 167
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isAod()Z

    move-result v14

    .line 168
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->isDisplayOn()Z

    move-result v15

    .line 169
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getDockState()I

    move-result v16

    .line 170
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getOrientation()I

    move-result v4

    invoke-static {v4}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->orientationType(I)I

    move-result v17

    .line 171
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getFoldState()I

    move-result v4

    invoke-static {v4}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->foldType(I)I

    move-result v18

    .line 172
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getOrderAndIncrement()I

    move-result v19

    .line 173
    invoke-static/range {p1 .. p1}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->toProtoWakeReason(Lcom/android/server/biometrics/log/OperationContextExt;)I

    move-result v20

    .line 174
    invoke-static/range {p1 .. p1}, Lcom/android/server/biometrics/log/BiometricFrameworkStatsLogger;->toProtoWakeReasonDetails(Lcom/android/server/biometrics/log/OperationContextExt;)[I

    move-result-object v21

    .line 175
    invoke-virtual/range {p1 .. p1}, Lcom/android/server/biometrics/log/OperationContextExt;->getIsMandatoryBiometrics()Z

    move-result v22

    .line 154
    const/16 v0, 0x59

    const/4 v11, -0x1

    move/from16 v1, p2

    move/from16 v4, p3

    move/from16 v5, p4

    move/from16 v8, p5

    move/from16 v6, p8

    move/from16 v7, p9

    move/from16 v2, p10

    invoke-static/range {v0 .. v22}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIIZIIIIZJIIIZZIIIII[IZ)V

    .line 176
    return-void
.end method

.method public logFrameworkNotification(II)V
    .locals 1
    .param p1, "action"    # I
    .param p2, "modality"    # I

    .line 277
    const/16 v0, 0x331

    invoke-static {v0, p1, p2}, Lcom/android/internal/util/FrameworkStatsLog;->write(III)V

    .line 279
    return-void
.end method

.method public reportFingerprintsLoe(I)V
    .locals 3
    .param p1, "statsModality"    # I

    .line 269
    const/4 v0, 0x5

    const/4 v1, -0x1

    const/16 v2, 0x94

    invoke-static {v2, p1, v0, v1}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIII)V

    .line 273
    return-void
.end method

.method public reportUnknownTemplateEnrolledFramework(I)V
    .locals 3
    .param p1, "statsModality"    # I

    .line 261
    const/4 v0, 0x2

    const/4 v1, -0x1

    const/16 v2, 0x94

    invoke-static {v2, p1, v0, v1}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIII)V

    .line 265
    return-void
.end method

.method public reportUnknownTemplateEnrolledHal(I)V
    .locals 3
    .param p1, "statsModality"    # I

    .line 253
    const/4 v0, 0x3

    const/4 v1, -0x1

    const/16 v2, 0x94

    invoke-static {v2, p1, v0, v1}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIII)V

    .line 257
    return-void
.end method

.method public unenrolled(IIII)V
    .locals 1
    .param p1, "statsModality"    # I
    .param p2, "targetUserId"    # I
    .param p3, "reason"    # I
    .param p4, "templateId"    # I

    .line 132
    const/16 v0, 0x3b0

    invoke-static {v0, p1, p2, p3, p4}, Lcom/android/internal/util/FrameworkStatsLog;->write(IIIII)V

    .line 137
    return-void
.end method
